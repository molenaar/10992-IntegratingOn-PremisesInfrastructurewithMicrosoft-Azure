...
#### Task 1: Install Docker Toolbox for Windows
  ...
  > **Note:** Due to limitations of the lab, we will install the older **Docker Toolbox**, instead of the preferred **Docker Desktop for Windows**. More information on Docker Desktop for Windows is available at https://docs.docker.com/docker-for-windows/install/.
  
1.   On MIA-CL1, start Internet Explorer and browse to **https://github.com/docker/toolbox/releases**.
2.   To download Docker Toolbox click: **DockerToolbox-18.09.3.exe** (or the current DockerToolbox-*xx.xx.x*.exe installer).
3.   In the download prompt click **Save** and when the download completes click **Run**.
5.   On the **Welcome** window, click **Next**.
6.   On the **Select Destination Location** window, click **Next**.
7.   On the **Select components** window, **_uncheck_** **Kitematic for Windows** and **Git for Windows**, and then click **Next**.
8.   On the **Select Additional Tasks** window, **_uncheck_** **Create a desktop shortcut**, and click **Next**.
9.   On the **Ready to Install** window, click **Install**.
10.   On the **_Would you like to install this device software?_** window, click **Don’t Install**
11.   On the **Completing the Docker Toolbox Setup Wizard** window, **_Uncheck_** **View Shortcuts in File Explorer**, and then click **Finish**.

#### Task 2: Create a Docker host in a Hyper-V virtual machine by using Docker Machine

1.   On MIA-CL1, click **Start**, in the Start menu, expand the **Windows System** folder, right-click **Command Prompt**, in the right-click menu, click **More** and then click **Run as administrator**.
...
3.  Wait for the new Microsoft Hyper-V virtual machine to be provisioned. This might take a few minutes. If the boot2docker ISO image is out-of-date, Docker Machine will initiate download of the latest version.
